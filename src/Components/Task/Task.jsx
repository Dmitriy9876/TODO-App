...
export default function Task({
  ...
  const updateTasks = (updateFunc) => {
    setTasks((currentTasks) =>
      currentTasks.map((task) => (task.id === id ? updateFunc(task) : task))
    );
  };
    ...
    <div className="view">
      ...
      <label htmlFor={`toggle-${id}`}>
        {isEditing ? (
          ...
          <button type="button" className={className} onClick={onToggleDone} tabIndex="0">
            {description}
          </button>
        )}
        <span className="description">
          ...
          />
          <span className="time">{`${Math.floor(time / 60)
            .toString()
            .padStart(2, '0')}:${(time % 60).toString().padStart(2, '0')}`}</span>
